...
def save_app_config(config: Dict) -> bool:
    """Salva configurações do aplicativo. Retorna True se salvou com sucesso."""
    try:
        ...
        st.warning(f"Não foi possível salvar configuração: {e}")
        return False
...
def check_ollama_online() -> Tuple[bool, str]:
    ...
    endpoints = [f"{base_root}/api/version", f"{base_root}/"]
    for url in endpoints:
        try:
            ...
            continue
    return False, "Ollama offline ou OLLAMA_BASE incorreto"
...
def list_ollama_models() -> Tuple[List[str], str]:
    try:
        ...
        return [], f"Erro ao listar modelos: {str(e)[:100]}"
...
def main() -> None:
    ...
    if models_error:
        st.sidebar.caption(f"⚠️ {models_error}")
                ...
                if not filtered_history:
                    ...
                    for idx, entry in enumerate(filtered_history):
                        ...
                        with st.container():
                            ...
                            # Expander de contexto (apenas para comparação de versões)
                            if is_comparison:
                                with st.expander(t(trans, "expanders.versions_compared", "🔍 Ver Versões Comparadas"), expanded=False):
                                    st.markdown(
                                        f"""
                                        <div style="
                            ...
                            with col_a:
                                if st.button(t(trans, "buttons.copy", "📋 Copiar"), key=f"copy_{idx}", use_container_width=True):
                                    ...
                                    context_part = f"**{t(trans, 'formatting.context_label', 'Contexto:')}**\n{entry['context']}\n\n" if is_comparison else ""
                                    full_text = f"**{reference}**\n\n{context_part}**{explanation_label}**\n{entry['explanation']}"
